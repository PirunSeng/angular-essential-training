...
export class MockXHRBackend implements HttpBackend {
  handle(request: any): Observable<HttpEvent<any>> {
    return Observable.create((responseObserver: Observer<any>) => {
      let responseOptions;
      switch (request.method) {
        case 'GET':
          if (request.urlWithParams.indexOf('mediaitems?medium=') >= 0 || request.url === 'mediaitems') {
            ...
            const mediaItems = this._mediaItems.filter(mediaItem => mediaItem.id === id);
            responseOptions = {
  ...
  _deleteMediaItem(id) {
    const mediaItem = this._mediaItems.find(item => item.id === id);
